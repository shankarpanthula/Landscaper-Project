...
SET BITS=64
SET USER_JDK64_FILE=%USERPROFILE%\.WebStorm2017.2\config\webstorm%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%USERPROFILE%\.WebStorm2017.2\config\webstorm%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%USERPROFILE%\.WebStorm2017.2\config\webstorm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_WEBIDE_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_WEBIDE.hprof" "-Xbootclasspath/a:%IDE_HOME%/lib/boot.jar" -Didea.paths.selector=WebStorm2017.2 %IDE_PROPERTIES_PROPERTY%
